<?php
    include_once '../../lib/config.php';
    //include_once '../../lib/fungsi.php';
    $id_minor = $_GET['id_minor'];
    $sqlminor = "SELECT * FROM t_minor WHERE id_minor='$id_minor'";
    $minorar = mysql_query( $sqlminor );
    $emp = mysql_fetch_array( $minorar );
?>

            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hid_minor="true">&times;</span></button>
                        <h4 class="modal-title" id_minor="myModalLabel">Hapus Data minor</h4>
                    </div>
                        <div class="panel-body">

                            <div class="row">
                                <div class="col-lg-6">
                                    <form>
                                    <div class="alert alert-danger">Apakah anda yakin ingin menghapus data ini ( <?php echo $emp['nama'];?>) ?</div>
                                        <div class="form-group">
                                            <input type="hidden" id="id_minor" name="id_minor" value="<?php echo $id_minor;?>">
                                            <button type="button" class="btn btn-primary save_submit" name="Submit" value="SIMPAN">&nbsp;&nbsp;&nbsp;Ya&nbsp;&nbsp;&nbsp;</button>
                                            <button type="button" class="btn btn-primary" data-dismiss="modal" aria-hid_minor="true">&nbsp;Batal&nbsp;</button>
                                        </div>
                                    </form>
                                </div>
                            </div>

                        </div>
                        <!-- /.panel-body -->
                    </div>
                    <!-- /.panel -->
                </div>

                <script type="text/javascript">
                  $(document).ready(function (){
                        $(".save_submit").click(function (e){
                            var id_minor = $('#id_minor').val();
                           $.ajax({
                                url: 'minor/minor_del_save.php?id_minor='+id_minor,
                                type: 'GET',
                                success: function (response){
                                      //alert('asuransi/asuransi_del_save.php?$id_minor='+$id_minor);
                                      $("#tableminor").load('minor/minor_load.php');
                                      alert('Data Berhasil Dihapus');
                                      $('#ModalDelete').modal('hide');
                                }
                            });

                    });
                });
              </script> 